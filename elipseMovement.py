...
def bind_pieza(parent_path,material):
    parent_prim = stage.GetPrimAtPathh(parent_path)
...
stage = Usd.Stage.CreateNew("robot_scene_two.usda")
...
#1. map to the elipses formula
radius_x = 4.0
radius_y = 5.0
altura_base = 3.0 #maybe for oscillation
...
#elipse 
for frame in range(frame_inicial,frame_final+1,2):
    progress = (frame-frame_inicial)/(frame_final-frame_inicial)
    angle = progress*vueltas*2*math.pi
    
    #2. plane to live 
    x = radius_x * math.cos(angle)
    y = radius_y * math.sin(angle)
    
    #3. derivate for the elipse
    dx = radius_x * -math.sin(angle)
    dy = radius_y * math.cos(angle)
    
    z = altura_base + amplitud_vertical * math.sin(angle*3)
    translateOp.Set((x,y,z),time=frame)
    
    #4.turning dx/dy into an angle with atan2 , because is -180 to 180
    angle_tangent = math.degrees(math.atan2(dy,dx))
    rotateZop.Set(angle_tangent,time=frame)
...
materialNube = crear_material("MaterialNube",(0.95,0.95,0.95))
...
UsdShade.MaterialBindingAPI.Apply(cloud_proto.GetPrim()).Bind(materialNube)
