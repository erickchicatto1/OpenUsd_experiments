...
#
# Version standalone (sin Omniverse) del script de hand trucks con cajas.
# Requiere: pip install usd-core numpy
# Genera un archivo escena.usd que puedes abrir con usdview o Blender.
#
# TODO EN UN SOLO ARCHIVO: incluye tambien la conversion de materiales MDL
# (OmniPBR, tipicos de los assets de NVIDIA) a UsdPreviewSurface, para que
# los hand trucks y cajas se vean con sus colores/texturas reales en usdview
# y Blender en vez de aparecer en gris.
#
# Unica dependencia externa: usd_scene_construction_utils.py debe estar en
    ...
    for mat_prim, shader_prim in found:
        ...
        for prim in Usd.PrimRange(root_prim):
            if not (prim.IsA(UsdGeom.Gprim) or prim.IsA(UsdGeom.Mesh)):
                continue
            # Aplicar el schema evita el warning "MaterialBindingAPI is not
            # applied on the prim" en assets legacy que solo autoraron la
            # relacion material:binding sin aplicar formalmente la API.
            binding_api = material_api.Apply(prim)
            bound, rel = binding_api.ComputeBoundMaterial()
            if bound and bound.GetPrim().GetPath() == mat_prim.GetPath():
                binding_api.Bind(preview_mat)
                if verbose:
...
leatherback_asset_path = r"D:\Robotics_AI\5.OpenUSD\robots_usd\leatherback\leatherback.usd"

BOX_ASSET_DIR = os.path.dirname(box_asset_path)
HAND_TRUCK_ASSET_DIR = os.path.dirname(hand_truck_asset_path)
LEATHERBACK_ASSET_DIR = os.path.dirname(leatherback_asset_path)
...
if not os.path.exists(leatherback_asset_path):
    raise FileNotFoundError(
        f"No se encontro el asset del leatherback: {leatherback_asset_path}\n"
        "Ajusta leatherback_asset_path a donde lo hayas guardado.")

# Configuracion de la animacion del leatherback (recorrido circular).
DRIVE_CENTER = (0, 0)      # centro del circulo, en el mismo espacio que la grilla
DRIVE_RADIUS = 550.0       # radio del recorrido (cm)
DRIVE_LAPS = 1             # numero de vueltas
DRIVE_SECONDS_PER_LAP = 8.0
DRIVE_FPS = 24
DRIVE_KEYFRAMES_PER_LAP = 32  # mas puntos = curva mas suave
...
def add_leatherback_with_drive_animation(
        stage, path: str,
        center=DRIVE_CENTER, radius=DRIVE_RADIUS, laps=DRIVE_LAPS,
        seconds_per_lap=DRIVE_SECONDS_PER_LAP, fps=DRIVE_FPS,
        keyframes_per_lap=DRIVE_KEYFRAMES_PER_LAP):
    """Referencia el asset del leatherback, convierte sus materiales MDL a
    UsdPreviewSurface, lo asienta en el piso, y le agrega una animacion de
    manejo en circulo (posicion + orientacion tangente al recorrido).

    La animacion se autora en operaciones xform separadas (sufijo 'drive')
    para no interferir con ningun posicionamiento estatico que quieras
    aplicarle despues con translate()/rotate_z() del utils.
    """
    car_prim = add_usd_ref(stage, path, usd_path=leatherback_asset_path)

    if DEBUG_INSPECT_MATERIALS:
        inspect_materials(stage, car_prim)
    convert_mdl_materials_to_preview(stage, car_prim, asset_dir=LEATHERBACK_ASSET_DIR, verbose=False)

    # Asentar la base en z=0 por si el pivote del asset no esta en el piso.
    car_min, _ = compute_bbox(car_prim)
    translate(car_prim, (0, 0, -car_min[2]))

    xformable = UsdGeom.Xformable(car_prim)
    translate_op = xformable.AddTranslateOp(opSuffix="drive")
    rotate_op = xformable.AddRotateZOp(opSuffix="drive")

    total_keyframes = int(keyframes_per_lap * laps)
    total_seconds = seconds_per_lap * laps
    for i in range(total_keyframes + 1):
        t = i / keyframes_per_lap  # fraccion de vuelta (puede pasar de 1 si laps>1)
        angle = 2 * math.pi * t
        x = center[0] + radius * math.cos(angle)
        y = center[1] + radius * math.sin(angle)
        heading_deg = math.degrees(angle) + 90  # tangente a la circunferencia
        time_code = (i / total_keyframes) * total_seconds * fps if total_keyframes else 0
        translate_op.Set(Gf.Vec3d(x, y, 0), Usd.TimeCode(time_code))
        rotate_op.Set(heading_deg, Usd.TimeCode(time_code))
...
# Leatherback manejando en circulo alrededor de la grilla de hand trucks.
add_leatherback_with_drive_animation(stage, "/scene/leatherback")

# Configurar el rango de tiempo del stage para que usdview/Blender detecten
# la animacion y muestren la linea de tiempo/timeline correctamente.
total_frames = DRIVE_KEYFRAMES_PER_LAP * DRIVE_LAPS
stage.SetStartTimeCode(0)
stage.SetEndTimeCode((total_frames / DRIVE_KEYFRAMES_PER_LAP) * DRIVE_SECONDS_PER_LAP * DRIVE_FPS if total_frames else 0)
stage.SetTimeCodesPerSecond(DRIVE_FPS)
stage.SetFramesPerSecond(DRIVE_FPS)
...
print("Nota: si al abrir en Blender/usdview no ves la animacion moverse,")
print("revisa que tu funcion export_stage() no descarte los metadatos de")
print("timeCode del stage (startTimeCode/endTimeCode/timeCodesPerSecond).")
